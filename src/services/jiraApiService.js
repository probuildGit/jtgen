...
const jiraApi = axios.create({
  baseURL: 'http://localhost:3001', // Use proxy server
  headers: {
...
export const createJiraTicket = async (ticketData) => {
  try {
    ...
    const response = await jiraApi.post('/create-ticket', ticketPayload);
...
export const uploadAttachment = async (issueKey, file) => {
  try {
    ...
    const response = await axios.post(
      `http://localhost:3001/upload-attachment/${issueKey}`,
      formData,
      {
        headers: {
          'Accept': 'application/json',
    ...
    return response.data.data; // Proxy returns { success: true, data: attachmentObject }
  } catch (error) {
...
export const testJiraConnectivity = async () => {
  try {
    const response = await jiraApi.get('/test-connectivity');
